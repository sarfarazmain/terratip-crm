...
def show_admin(users_df):
    t1, t2, t3, t4 = st.tabs(["➕ New User", "✏️ Edit User", "📥 Upload CSV", "📤 Export Junk"])
    with t1:
    ...
    with t4:
        st.subheader("Export Junk for Meta Ads Exclusion")
        st.info("Exports leads marked as Junk, Broker, or Invalid for 'Custom Audience Exclusion' in Meta Ads.")
        
        if st.button("Generate Exclusion List"):
            try:
                data = leads_sheet.get_all_records()
                df = pd.DataFrame(data)
                ...
                if not junk_df.empty:
                    ...
                    st.download_button(
                        label="⬇️ Download Exclusion List (CSV)",
                        data=csv,
                        file_name=f"meta_exclusion_list_{get_ist_date()}.csv",
                        mime="text/csv",
                    )
                else:
                    st.warning("No junk leads found in the database yet.")
