        ...
        if uploaded_file is not None and st.button("Start Upload"):
            if not selected_agents: st.error("⚠️ Please select at least one agent.")
                ...
                try:
                    ...
                    # --- META DATA EXTRACTION LOGIC ---
                    # Find columns that look like questions (contain Hindi or keywords)
                    meta_question_cols = []
                    for c in df_up.columns:
                        if c not in [name_col, phone_col] and is_likely_question(c):
                            meta_question_cols.append(c)
                    ...
                    for idx, row in df_up.iterrows():
                        ...
                        if len(p_clean) >= 10:
                            ...
                            if p_last_10 not in existing_phones_set:
                                ...
                                # Compile Meta Data into Note
                                meta_note = ""
                                if meta_question_cols:
                                    meta_data_parts = []
                                    for qc in meta_question_cols:
                                        ans = str(row[qc]).strip()
                                        if ans and ans.lower() != 'nan':
                                            # Clean up the question header for brevity
                                            short_q = qc[:15] + ".." if len(qc) > 15 else qc
                                            meta_data_parts.append(f"{short_q}: {ans}")
                                    if meta_data_parts:
                                        meta_note = f"[META FORM] {' | '.join(meta_data_parts)}"
                                
                                new_row = [new_id, ts, row[name_col], p_clean, "Meta Ads", "", assigned_person, "Naya Lead", "", ts, "", "", meta_note, "", ""]
